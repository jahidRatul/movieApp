...
class CountryAllMoviesModel {
  bool status;
  String appUrl;
  Data data;

  CountryAllMoviesModel({this.status, this.appUrl, this.data});

  CountryAllMoviesModel.fromJson(Map<String, dynamic> json) {
    this._fromJson(json);
  }
  ...
  Future<void> callApi({String id}) async {
    try {
      Map<String, dynamic> res =
          await HttpHelper.get(AppUrls.countryAllMoviesUrl + id);
      if (res == null) return;
...
class Pivot {
  ...
  Pivot({this.genreId, this.videoId, this.createdAt, this.updatedAt});
